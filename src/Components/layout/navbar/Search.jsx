    ...
    <div className="w-4/12">
      <div className="relative" ref={dropdownRef}>
        <input
          ...
          className="custom-input bg-[#00000063] shadow-md border-none ring-0"
        />
        ...
        {isOpen && (
          <ul className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto text-primaryDarker">
            {isLoading ? (
...
const Search = ({ user }) => {
  ...
  const { data: plantResponse, isFetching: isPlantsLoading } =
    usePlantByDiseases(selectedDisease?.value);
  const diseases = response?.items || [];
  const plantsData = plantResponse?.items || [];
